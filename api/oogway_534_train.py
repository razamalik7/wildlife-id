    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            ...
            if len(self.valid_pairs) > 0:
                ...
                selected_idx = np.random.choice(len(self.valid_pairs), size=n_pairs, replace=False, p=PAIR_PROBS[:len(self.valid_pairs)])
...
class HierarchicalMultiTaskLoss(nn.Module):
    ...
    def forward(self, predictions, species_targets, family_targets, class_targets):
        ...
        # Penalize if predicted species doesn't match predicted family
        species_probs = F.softmax(species_logits, dim=1)
        family_probs = F.softmax(family_logits, dim=1)
        
        # For each sample, get family prediction
        _, pred_family_idx = torch.max(family_logits, 1)
        
        # Compute expected family probabilities from species predictions
        # (aggregate species probs by their family)
        consistency_loss = 0.0
        batch_size = species_logits.size(0)
        
        for i in range(batch_size):
            # Get top predicted species
            top_species_idx = species_probs[i].argmax()
            species_name = self.class_names[top_species_idx]
            true_family = SPECIES_TO_FAMILY.get(species_name, "unknown")
            
            # Get predicted family
            pred_family_idx_i = pred_family_idx[i].item()
            # This would need family index to name mapping, but for simplicity:
            # We penalize KL divergence between aggregated species→family and direct family prediction
            # Skip for now to keep implementation simple, but the concept is:
            # consistency_loss += KL(species_aggregated_family_dist, family_dist)
        
        # For now, skip explicit consistency loss - the shared features + joint training
        # should implicitly enforce consistency
...
if __name__ == "__main__":
    ...
    print("="*70)
    b3_val, b3_tta = train_model('b3')
    results['b3'] = {'val': b3_val, 'tta': b3_tta}
    
    # Train ConvNeXt
    print("\n\n" + "="*70)
    print("TRAINING MODEL 2/2: ConvNeXt-Tiny")
    print("="*70)
    convnext_val, convnext_tta = train_model('convnext')
    results['convnext'] = {'val': convnext_val, 'tta': convnext_tta}
    ...
    print(f"  Val Acc: {results['b3']['val']:.4f} ({results['b3']['val']*100:.2f}%)")
    print(f"  TTA Acc: {results['b3']['tta']:.4f} ({results['b3']['tta']*100:.2f}%)")
    print(f"\nConvNeXt-Tiny:")
    print(f"  Val Acc: {results['convnext']['val']:.4f} ({results['convnext']['val']*100:.2f}%)")
    print(f"  TTA Acc: {results['convnext']['tta']:.4f} ({results['convnext']['tta']*100:.2f}%)")
    print(f"\nEnsemble (avg): {(results['b3']['tta'] + results['convnext']['tta'])/2:.4f}")
    print("\n💾 Models saved:")
    print("  - oogway_b3_best.pth (best epoch)")
    print("  - oogway_b3_swa.pth (SWA averaged)")
    print("  - oogway_b3_final.pth (after FixRes)")
    print("  - oogway_convnext_best.pth")
    print("  - oogway_convnext_swa.pth")
    print("  - oogway_convnext_final.pth")
    print("\n🎯 Ready for targeted fine-tuning on confused pairs!")
    print("="*70)
